...
// import { useGetSingleFoodQuery, useUpdateFoodMutation } from "@/redux/features/food/food.api";
...
// function FileUpload({ value, onChange, existingUrl, accept = "image/*", label = "Upload Image" }: any) {
...
//   // Assuming your backend supports getting by slug
//   // If not — change endpoint or pass _id instead
//   const { data: foodData, isLoading: isFoodLoading, isError: foodError } = useGetSingleFoodQuery(slug);

//   const [updateFood, { isLoading: isUpdating }] = useUpdateFoodMutation();
//   const { data: catData, isLoading: isCategoriesLoading, isError: catError } = useGetAllCategoriesQuery({});
...
//       typeof food.category === "object" && (food.category as any)?._id 
//         ? (food.category as any)?._id 
...
//       price: food.price || 0,
//       offerPrice: food.offerPrice ?? undefined,
//       image: food.image || undefined,
//       status: food.status || FoodStatus.ACTIVE,
//       ingredients: food.ingredients?.length ? food.ingredients : [],
//       totalStock: food.totalStock || 0,
//       unit: food.unit || "plate",
//     });

//     // Extra safety: force setValue after reset
//     if (selectedCategoryId) {
//       form.setValue("category", selectedCategoryId);
//     }
//   }
// }, [food, categories, form]);  // ← categories dependancy যোগ করুন
...
//       <h1 className="text-3xl font-bold mb-6 text-[#002047]">Update Food Item</h1>
...
//         value={field.value || ""}  // ← null/undefined avoid করুন
//         disabled={isCategoriesLoading || catError || categories.length === 0}
//       >
//         <FormControl>
//           <SelectTrigger>
//             <SelectValue
...
//           {/* Image & Description */}
//           <Card>
//             <CardHeader><CardTitle>Image & Description</CardTitle></CardHeader>
...
//           {/* Ingredients */}
//           <Card>
//             <CardHeader><CardTitle>Ingredients (Optional)</CardTitle></CardHeader>
//             <CardContent>
//               {ingredients.length > 0 ? (
//                 <div className="space-y-5">
//                   {ingredients.map((item, index) => (
//                     <div key={item.id} className="grid sm:grid-cols-5 gap-4 border p-4 rounded-lg relative">
//                       <Button
//                         type="button"
//                         variant="destructive"
//                         size="icon"
...
//                       <FormField name={`ingredients.${index}.name`} control={form.control} render={({ field }) => (
...
//                       <FormField name={`ingredients.${index}.price`} control={form.control} render={({ field }) => (
//                         <FormItem>
//                           <FormLabel>Price</FormLabel>
//                           <FormControl>
//                             <Input type="number" {...field} onChange={e => field.onChange(parseFloat(e.target.value) || 0)} />
...
//                       <FormField name={`ingredients.${index}.unit`} control={form.control} render={({ field }) => (
//                         <FormItem>
...
//   image: z.any().optional(), // ← Changed: more flexible for update form
//   status: z.nativeEnum(FoodStatus),
...
//       if (selectedCategoryId) {
//         form.setValue("category", selectedCategoryId, { shouldValidate: true });
//       }
...
//       // Only append new image if a file was uploaded
//       if (values.image instanceof File) {
//         formData.append("image", values.image);
//       }
...
const foodSchema = z.object({
  ...
  description: z
    ...
    .min(10, { message: "Description must be at least 10 characters" })
    .max(500),
  image: z.any().optional(), // string (url) or File
  status: z.nativeEnum(FoodStatus),
  ingredients: z.array(z.string()).optional(),
  unit: z.string().min(1, { message: "Unit is required" }),
...
export default function UpdateFoodForm() {
  ...
  useEffect(() => {
    if (food && categories.length > 0) {
      ...
      // Prepare variants – assuming backend now returns variants array
