...
//                   className={`px-2 py-1 text-sm rounded-full border transition-colors ${removedDefaults.has(ing.name)
...
export default function FoodDetailsDialog({ food, open, onOpenChange }: Props) {
  ...
  // Foods for Farcita pizza - only when condition matches
  // useEffect(() => {
  //   if (!open) return;
  //   if (food.category?.title === "Pizza a Metro" && food.name === "Farcita") {
  //     const fetchFoods = async () => {
  //       try {
  //         const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/food/all-foods?limit=400`);
  //         const data = await res.json();
  ...
  const extrasTotal = useMemo(() => {
    ...
    // যদি Pizza a Metro হয়, সব extra ingredient 4€ ধরে নাও
    if (food?.category?.title === "Pizza a Metro") return extras.length * 4;
