...
// export default function PosCartSheet({ isOpen, onClose }: PosCartSheetProps) {
...
//         <SheetContent
//           side="right"
//           className="max-w-md p-0 flex flex-col h-full [&>button]:hidden overflow-y-auto"
//         >
//           {/* Header */}
...
//                                 <button
//                                   onClick={() =>
//                                     handleRemoveIngredient(item.id, extra.name)
//                                   }
...
//           {/* Form Section */}
//           <div className="p-5 border-t bg-gray-50 flex-none">
...
export default function PosCartSheet({ isOpen, onClose }: PosCartSheetProps) {
  ...
  // When subtotal drops below 7, force PICKUP — but only if currently DELIVERY
  useEffect(() => {
  ...
  return (
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent
        side="right"
        // KEY FIX: Remove flex-col + overflow-y-auto from the root.
        // Instead, use flex-col with a fixed height so inner sections can scroll independently.
        className="max-w-md p-0 flex flex-col h-full [&>button]:hidden"
      >
        ...
        {/* ── Scrollable body: cart items THEN form ── */}
        <div className="flex-1 overflow-y-auto">
          ...
          {/* ── Order Form (directly below cart items, inside the same scroll area) ── */}
          <div className="p-5 border-t bg-gray-50">
            ...
            <Form {...form}>
              <form
                ...
                className="space-y-6"
              >
                {/* Delivery Option — controlled via form state, not local state */}
                <FormField
                  control={form.control}
                  name="deliveryOption"
                  render={({ field }) => (
                    <FormItem className="space-y-3">
                      ...
                      <FormControl>
                        <RadioGroup
                          value={field.value}
                          onValueChange={(val) =>
                            field.onChange(val as DeliveryOption)
                          }
                          className="flex flex-row gap-6"
                        >
                          <FormItem className="flex items-center space-x-3 space-y-0">
